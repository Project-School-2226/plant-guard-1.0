...
const char* serverUrl = "http://172.168.0.69:3500/sensors-data/post-data-to-server"; // Adjust the server URL
...
WebServer server(80); // Define a WebServer instance listening on port 80
...
class SensorValues {
  private: 
    se_p moisture_pin;
    se_p temperature_pin;

  public: 
    SensorValues(se_p humidity_pin, se_p temperature_pin) {
...
String formatData(float N, float P, float K, float temp, float soilMoisture, float humidity) {
...
void handleGetRequest() {
  ...
  String jsonData = formatData(sensorData[0], sensorData[1], sensorData[2], sens_vals.getTemperature(), handleMoisturePer(sens_vals.getSoilMoisture()), sens_vals.getHumidity());
...
void loop() {
  ...
  float humidity = sens_vals.getHumidity();
  ...
  sensorData[5] = humidity; // Assuming humidity is stored at index 5
